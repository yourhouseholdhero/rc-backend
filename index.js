...
export default function App() {
  ...
  const handleAnalyze = async () => {
    setLoading(true);
    try {
      const analyzeRes = await axios.post('https://rc-backend-b307.onrender.com/api/analyze', {
        title
      });
      const qrRes = await axios.post('https://rc-backend-b307.onrender.com/api/qrcode', {
        title
      });
      setResult({
        ...analyzeRes.data,
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('https://rc-backend-b307.onrender.com/api/sync', {
        ...result,
        salePrice,
        payout,
